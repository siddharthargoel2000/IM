...
df = pd.read_csv(r"C:\Users\SIDDHARTH GOEL\OneDrive\Desktop\Inventory Management\data_2.csv")

#Add items to dictionaries
unit_price = df['Price'].dropna()

description = df['Product'].dropna()

stock = df['Stock'].dropna()

#First line of the file is the number of items
no_items  = len(description)
...
while(c!= "q" or c!= "Q"):
    ...
    if(c=="q" or c=="Q"):
        ...
        unit_price = unit_price.append(pd.Series([p_pr]), ignore_index=True)
        description = description.append(pd.Series([p_desc]), ignore_index=True)
        if(p_stock > -1):
            stock = stock.append(pd.Series([p_stock]), ignore_index=True)
        else:
            p_stock = 0
            stock = stock.append(pd.Series([p_stock]), ignore_index=True) 
            print("The stock of an item cannot be negative, the stock has been set to 0.")
        print()
        print("Part number: ",p_no," Description: ",description[p_no]," Price: ",unit_price[p_no]," Stock: ",stock[p_no])
        ...
        if(p_no in unit_price):
            ...
            unit_price = unit_price.replace(to_replace = [unit_price[p_no]], value = [p_pr])
            description = description.replace(to_replace = [ description[p_no]], value = [p_desc])
            stock = stock.replace(to_replace = [stock[p_no]], value = [p_stock])
        ...
        p_no = int(input("Enter part number: "))
        if(p_no in unit_price):
            are_you_sure = input("Are you sure you want to remove : {} ,(y/n)? ".format(description[p_no]))
            if(are_you_sure=="y" or are_you_sure=="Y"):
                unit_price = unit_price.drop([p_no])
                description = description.drop([p_no])
                stock = stock.drop([p_no])
        ...
        print()
        new_df = {'Items' : description,'Stock' : stock, 'Price':unit_price }
        ...
        if(p_no in unit_price):
            print()
            print("Part number: ",p_no," Description: ",description[p_no]," Price: ",unit_price[p_no]," Stock: ",stock[p_no])
            if(stock[p_no]<3 and stock[p_no]!=0):
                print("Only ",stock[p_no]," remaining! Hurry!")
            print()
        else:
            print("Sorry we don't have such an item!")
            print()
        ...
        if(p_no in unit_price):
            ...
            if(stock_current>0):
                ...
                print(description[p_no],"added to cart: ","$",item_price)
        ...
        if(are_you_sure=="y"):
            ...
            if(p_no in cart):
                ...
                cart.pop(j)
                print(description[p_no],"removed from cart: ")
                print()
            else:
                print()
...
new_df.to_csv(r'C:\Users\SIDDHARTH GOEL\OneDrive\Desktop\Inventory Management\Stocks.csv')